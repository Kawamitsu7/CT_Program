...
for i in trange(num_of_projections, desc='Read_Img', leave=True):
...
# cProfile.run('astra.algorithm.run(algorithm_id)',filename='only_astra.prof')
...
# Save reconstruction.
for i in trange(detector_rows, desc='Write_Img', leave=True):
	im = reconstruction[i, :, :]
	im = np.flipud(im)
