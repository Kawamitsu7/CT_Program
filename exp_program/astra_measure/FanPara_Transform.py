...
def main(f,num):
	...
	para_im = F2P_acc(input_f,Lo,Ld,px_size)
	
	cv2.imwrite("FP_Trans"+ os.sep +"F2P_"+ str(num) +".tif", para_im.astype(np.uint16))

	return para_im

def F2P_acc(I_im,Lo,Ld,px_size):
	delta_theta = 360 / I_im.shape[0]
	theta_p = np.arange(I_im.shape[0]).reshape(I_im.shape[0],1)
	Xp = np.arange(I_im.shape[1]).reshape(1,I_im.shape[1])

	# theta_fの計算
	real_Xp = (Xp - I_im.shape[1] / 2 - 1 / 2) * px_size
	alpha = np.arctan(real_Xp / np.sqrt(Lo ** 2 - real_Xp ** 2))
	theta_f = ( theta_p * delta_theta - np.rad2deg(alpha) ) / delta_theta

	# Xfの計算
	real_Xf = Ld * real_Xp / np.sqrt(Lo ** 2 - real_Xp ** 2)
	Xf = real_Xf / px_size + I_im.shape[1]/2 + 1/2

	# fold_back
	theta_f = np.where(theta_f < 0, I_im.shape[0] + theta_f, theta_f)
	theta_f = np.where(theta_f >= I_im.shape[0], theta_f - I_im.shape[0], theta_f)
	Xf = np.where(Xf < 0, 0, Xf)
	Xf = np.where(Xf >= I_im.shape[1], I_im.shape[1] - 1,Xf)

	# 線形補完
	y = np.floor(theta_f).astype(int)
